...
int  main(int argc, char **argv)
  {
    ...
    bzero(( char *) &serverAddr , sizeof (serverAddr)); // set bytes in memory location to zero.
    serverAddr.sin_family   = AF_INET;
    printf("server family: %d\n", AF_INET);
    serverAddr.sin_addr.s_addr  = inet_addr(argv[1]); // converts the Internet host in IPv4 numbers-and-dots notation  into  binary  data  in network byte order
    serverAddr.sin_port  = htons(serverPortNumber); //converts from host byte order to network byte order
    printf("sin_port: %x\n", serverAddr.sin_port);
