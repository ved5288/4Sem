...
;===========YOUR CODE==============================
;==================================================
	; Privilege 0 data segment starting at 0xe000
	mov ax,0x18 
	mov fs,ax
    mov dword [fs:0x0],0xdead 	;0xe000

	;segment DS that holds the current user password
	mov ax,0x68
	mov ds,ax
	;segment ES that holds the hashed passwords
	mov ax,0x70
	mov es,ax
	
	;Accepting the current user password. 
	;Password entered by the user is 0x50.
	mov dword [ds:0x0],0x00000050 
	;spawn a user Task that is coded by the user.
	call  0x82:0 
	
	;End of User program. Jump to chanHappy
	jmp chanHappy


;===========User Task (PL3 task)=====================
;========Question Q1=================
;Make entries for the User Task TSS at location
;pointed by the GDT entry 0x82.
;========END OF QUESTION Q1==========
align 0x400;
	
	;Privilege 0 data segment starting at 0xe000
	;Privileged and Elevated users must be granted
	;access.
	mov ax,0x18 
	;Below Statement causes a GPF exception for Elevated
	;and Unprivileged users.
	mov fs,ax
    mov dword [fs:0x0],0xfeed 	;0xe000

	;Call to PL0 code segment
	call 0xeb:0
	iret
	;User Task returns back 

;===========Interrupt Service Routine for GPF=============
align 0x100;
    pop ebx	
    
	;segment that holds the current user password
	mov ax,0x68
	mov ds,ax
	;segment that holds the precalculated hashed passwords
	mov ax,0x70
	mov es,ax
	
	;Get the current user password from 0xb000
	mov dword eax,[ds:0x0]
	; calculate the hash by adding 0x230
	add eax,0x230 

	
	;=================QUESTION Q2 =============
	;Write a loop to compare the precalculated hash 
	;passwords present in 0xb100 against the above
	;calculated current user password (present in eax)
	;=====End of QUESTION Q2===============
	mov esi,0x0; initialize loop variable
	mov edi,0x8
continue:	
	cmp eax,[es:esi]
	je  grant
...
grant:	
	...
	;=============Question 3 =======================
	;a) Write below code to give him access to PL 0 data by changing
	; it's priviliege level to 3.
	;b) Also set up a call gate at e8 entry of GDT to access PL0 code.
	;===============================================
	xor eax,eax
	mov ax,0x10
	mov gs,ax
	...
	mov al,11110010b
	mov [gs:0x201d],al
	;setting up call gate entry
	mov dword [gs:0x20e8],0x00080600
	mov dword [gs:0x20ec],0x0000ec00

	; mov ax,0x30
	; mov fs,ax
 ;    mov dword [fs:0xc],0xaff0c 	;0xe300
	iret

dontgrant:
	;Do not do anything in case we are not granting access
	;This case is for unprivileged users.
	mov ax,0x30
	mov fs,ax
    mov dword [fs:0x8],0xdeadbabe 	;0xe300
	jmp chanSad
...
align 0x100;
	...
	mov gs,ax
	mov ebx,[gs:0x80]
	cmp dword ebx,0xcafe
	jne failed
	mov dword [gs:0x600] , 0xbabecafe ;0xb600
	jmp done
failed:
	mov dword [gs:0x600] , 0xdeadcafe
done:
	retf
;----------------------------YOUR CODE ENDS here-----------------
...
;===============================
; Task State Segment of Code 4 (Q1 Modification!)
;===============================
; Address = 0xb300
;===============================
dd 0x0000	; ptl
...
;===================================================
dd 0xb0000070, 0x0040b200	;#0x68	#Tss	#0
dd 0xb1000070, 0x00409100	;#0x70	#Tss	#0
...
dd 0xa4000500, 0x0040fa00	;#0xb8	#Code	#3
...
;===================================================
dd 0x00000000, 0x00000000    ;#0xe8	#Call	#3 
;===================================================
