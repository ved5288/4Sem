...
;==================================================
	mov AX,0x18	
	mov DS,AX	;ds starts from 0xb000
	mov dword [DS:0x08],0x00
	mov AX,0x20
	mov SS,AX   ;ss starts from 0xb100 (Expand up, R/W)
	mov ESP,0x50
	mov EBP,0x0
    mov dword [DS:0x00],0xecad
...
;=====ISR DIV0(INT 0)========
	mov BX,0x1
	mov dword [DS:0x08],0x01
	iretd

align 0x100
;=====ISR OVERLFOW(INT 4)====
	;==indicate overflow to user==
	mov dword [DS:0x20],0x0F
	mov dword [DS:0x08],0x02
	iretd

align 0x100
;ISR USER DEFINED(INT 31)
	;====indicate interrupt======
	mov dword [DS:0x30],0xDEF
	mov dword [DS:0x08],0x03
	iretd

align 0x100
;ISR GPF(INT 13)
	;mov BX,0x1
	mov dword [DS:0x08],0x04
	mov AX,0x30
	mov GS,AX
	mov byte [GS:0x2d],0x92
	iretd
...
; Offset 0x20
;Stack Segment Descriptor of type 1, random DPL and Base
...
dw 0x0800	;Limit 15->0
dw 0xb200	;Base 15->0
db 0		;Base 23->16
db 10010000b;P=1 DPL=0 S=1 Type=0000(R)
db 01000000b;G=0 W=1 Unused=0 AVL=0
db 0x0		;Base 31->24
; Offset 0x30
;Data Segment Descriptor of type 3, random DPL and Base
 dw 0x1000	;Limit 15->0
 dw 0xd000	;Base 15->0
 db 0		;Base 23->16
 db 10010010b;P=1 DPL=0 S=1 Type=0000(R)
 db 01000000b;G=0 W=1 Unused=0 AVL=0
 db 0x0		;Base 31->24
; Offset 0x38
;Data Segment Descriptor of type 4, random DPL and Base
 dd 0xcf70008f , 0x409200
;------------KAMA - PUT CODE SEGMENT DESCRIPTOR FOR ISR------

; Offset 0x40
dw 0x0600	;Limit 15->0
dw 0xa400	;Base 15->0
db 0		;Base 23->16
db 10011010b;P=1 DPL=0 S=1 Type=1010(R/W)
db 01000000b;G=0 W=1 Unused=0 AVL=0
db 0x0		;Base 31->24
...
;--------FILL UP IDT ENTRY FOR Type 13 HERE------
 dw 0x300  ; Offest <- [15:0]
 dw 0x40 ; SS <- [31:16]
 db 0x0
 db 10001111b ;P <- 1 , DPL <- 00 ,D <- 1
 dw 0x0
...
;Exception gate for type 30
 ...
 db 0xd1
 db 0xa5
;Exception gate for type 31
 dw 0x200  ; Offest <- [15:0]
 dw 0x40 ; SS <- [31:16]
 db 0x0
 db 10001110b ;P <- 1 , DPL <- 00 ,D <- 1
 dw 0x0
;Exception gate for type 32
 ;dw 0x300  ; Offest <- [15:0]
 ;dw 0x40 ; SS <- [31:16]
 ;db 0x0
 ;db 10001110b ;P <- 1 , DPL <- 00 ,D <- 1
