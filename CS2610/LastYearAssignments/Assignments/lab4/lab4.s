...
;==================================
	;======Switch to PL 1 Task=====
	mov dword [0x600] , 0xface
	call 0x70:0 ;== call 0x70 with RPL 0==
	jmp chanHappy
	
	;==Will be called by call Gate==
	align 0x100
	mov ax,0x18
	mov gs,ax
	mov dword [gs:0x40], 0xabcdabcd
	retf
...
;==================================
	;====Switch to PL 2 Task====
	mov ax,0x21  ;segment selector: 20, RPL =0x01
	mov gs, ax       
	mov dword [gs:0x20], 0xfaceb00c ;; Address = 0xe120
	call 0x79:0  ;== call 0x78 with RPL 1==
	; Return to PL 0 Task
	iret
...
;==================================
	;========Switch to PL 3 Task=====
	mov ax,0x2a ;segment selector: 28, RPL =0x10
	mov gs, ax       
	mov dword [gs:0x20], 0xcafebabe ; Address = 0xe220
	call 0x82:0 ;== call 0x80 with RPL 2==
	iret
...
;==================================
	...
	;IDT index: 13, IDT Address: 0xd210 + (13*8bytes) = 0xd278
	;Entry at 0xd278: Task Gate -> seg selector 0x88
	;TSS starts at 0xb400
	;CS in the TSS starts at offset 76(0x4c) i.e. 0xb44c
	;CS points to seg selector 0x8
	;Segment base: 0xa000
	;EIP in the TSS starts at offset 32(0x20) i.e. 0xb420
	;EIP: 0x800
	;CS:EIP --> 0xa800
	;====RECOVERED from FAULT========================
	mov ax,0x33      ;segment selector: 30, RPL =0x11
	mov gs, ax       
	mov dword [gs:0x20], 0xcafeb00c ; Address = 0xe320
	iret
...
;==================================
	pop ebx ;Pop the Error Code from stack
	;=====Test for enabling hack (VBoX Issue)======
	cmp dword ebx, 0x90909090
	je hack
	;=====Actual GPF Handler=======================
	and ebx, 0xfffffff8
	add ebx, 0x2004
	mov cx, 0x10
	mov es, cx ; ES points to 0xb000
	;=<Getting the descriptor for the faulting task>=
	;[0xes:ebx] -> 0xb000 + 0x2004 + "sel from Error Code"(i.e. 0xe0)
	bts dword [es:ebx], 14 ; change DPL to 3 
	jmp donegpf
	;===========Hack================================
hack:
	mov cx, 0x10
	mov es, cx ; ES points to 0xb000
	bts dword [es:0x20e4], 14 ;Getting the descriptor for the faulting task

	;=====END of Fault Handling========================
donegpf:
	mov ax,0x18
	mov gs,ax
	mov dword [gs:0x20], ebx   ;==push some debug info
	iret
