...
;===========YOUR CODE==============================
;==================================================
;your code for Fibonacci series should start from here;
;you need to properly set up the GDT entries (base, limit, permissions etc) for accessing different segments
;you need to also setup the the stack and ESP properly for calling functions.


; PN- a) the given entries in GDT are just samples, dont use the entries blindly.
; Please ensure the entries are proper before executing the code.
; b) In this template you need to take care only the code, data and GDT entries. Rest of
; the portions you dont need to touch (and it's advisable not to tamper those).
...
fibonacci :
	...
	pop ebx		; ebx stores n
	push eax	; return address pushed back
	
	fib1 :
		cmp ebx,0x1 		; check if n==1
		jg fib2			; if greater than 1 jump to fib2
		mov eax,[ds:0x0]	; if equal to 1, eax stores 1 [ fib(n) ]
		cmp esi,ebx
		je insert_in_right_segment
		ret
		
		

	fib2 :
		cmp ebx,0x2		; check if n==2
		jg recursion		; if greater than 2 jump to recursion
		mov eax,[ds:Ox4]	; if equal to 2, eax stores 1 [ fib(n) ]
		cmp esi,ebx
		je insert_in_right_segment
		ret

	recursion :
		mov ecx,ebx		; ecx and ebx store n
		sub ecx,0x2		; ecx stores n-2
		
		
		push ecx		; n-2 is pushed
		push ecx		; n-2 is pushed
				
		call fibonacci
			; after this call, eax will store fib(n-2)
			; the top most element in stack will be n-2
		

		mov ebx,eax		; ebx stores fib(n-2)
		pop ecx			; ecx stores n-2
		...
		call fibonacci
			; after this call, eax will store fib(n-1)
			; the top most element in stack will be fib(n-2)
			; the second element will be n-1
...
dd 0x1	; fib(2)
dd 0x10	; n
;db 0x20 ;garbage data you need to change
;db 0x40 ;garbage data you need to change

; if the align shifts from 0xb008 to 0xc000 or to 0xc008.
...
; Default Data Segment Descriptor of type 2, DPL = 0 (Index 0x10)
 ...
 db 0x40
 db 0x0
;------ADD YOUR SEGMENT DESCRIPTORS - Here -----
